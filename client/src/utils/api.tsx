import { PhrasesCSV } from "@/utils/models";
...
export const update_user_stock = async (
  stock_names: string[],
  emplid: string
) => {
...
export const addSymbolTab = async (symbol: string, tab: string) => {
  const response = await fetch(
    `${backendUrl}/ticket?ticket_str=${symbol}&tab=${tab}`,
    {
      method: "POST",
    }
  );
...
export const addSymbol = async (symbolString: string) => {
  const response = await fetch(
    `${backendUrl}/ticket?ticket_str=${symbolString}`,
    {
      method: "POST",
    }
  );
...
export const getPhrases = async (table_name: string) => {
  ...
  const response = await fetch(
    `${backendUrl}/phrases?table_name=${table_name}`
  );
  const data = await response.json();
...
export const removePhraseKeywordAndTable = async (
  keyword: string,
  table_name: string
) => {
  const response = await fetch(
    `${backendUrl}/phrases?keyword=${keyword}&table_name=${table_name}`,
    {
      method: "DELETE",
    }
  );
...
export const addCategory = async (categoryString: string) => {
  const response = await fetch(
    `${backendUrl}/category?name=${categoryString}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
